...
# Teto aproximado de entradas no stream. O XACK tira do PEL mas NÃO remove a
# entrada do stream — sem trim ele cresceria pra sempre. MAXLEN ~ poda as antigas
# (já processadas) de forma barata. 10k folga de sobra (reclaim roda a cada 30s,
# então nenhuma entrada pendente fica viva por 10k novas). Tunável por env.
STREAM_MAXLEN = int(os.getenv("DISPATCHER_STREAM_MAXLEN") or 10000)

# Move até max_batch membros vencidos (score <= now) do ZSET (KEYS[1]) pro
# stream (KEYS[2]), com MAXLEN ~ (ARGV[3]) pra podar entradas antigas. Atômico:
# quem chegar primeiro no Redis remove+publica; as outras réplicas já veem o ZSET
# sem o membro. Retorna quantos promoveu.
_PUMP_LUA = """
local n = 0
local due = redis.call('ZRANGEBYSCORE', KEYS[1], '-inf', ARGV[1], 'LIMIT', 0, tonumber(ARGV[2]))
for _, member in ipairs(due) do
  redis.call('ZREM', KEYS[1], member)
  redis.call('XADD', KEYS[2], 'MAXLEN', '~', ARGV[3], '*', 'conv', member)
  n = n + 1
end
return n
"""
...
async def pump_due(max_batch: int = 200) -> int:
    ...
    return int(await _pump_script(
        keys=[DUE_KEY, READY_STREAM], args=[time.time(), max_batch, STREAM_MAXLEN]
    ))
